...
const float WIDTH_ = 0.5f;		//太さ
const int LENGTH_ = 30;		//長さ
...
{
	const float POSITION_ADJUSTMENT = 2.0f;	// ポリラインのy座標調整値

	pos.y += POSITION_ADJUSTMENT;
	...
	//頂点データを作るための配列を準備
	VERTEX_POLY* vertices = new VERTEX_POLY[LENGTH_ * 2];
	...
	for (int i = 0; i < LENGTH_; i++ )
	{
		...
		VERTEX_POLY vertex1 = { pos, XMFLOAT3((float)i / LENGTH_, 0, 0) };

		XMStoreFloat3(&pos, vPos - vArm);
		VERTEX_POLY vertex2 = { pos, XMFLOAT3((float)i / LENGTH_, 1, 0) };

		int s = sizeof(VERTEX_POLY);
	...
	bd_vertex.ByteWidth = sizeof(VERTEX_POLY) * LENGTH_ * 2;
...
HRESULT PolyLine::Load(std::string fileName)
{
	...
	cb.ByteWidth = sizeof(CONSTANT_BUFFER_POLY);
...
void PolyLine::Draw()
{
	...
	//頂点バッファ
	UINT stride = sizeof(VERTEX_POLY);
	UINT offset = 0;
